...
Enhanced with call stack, method calls, and persistent variable support.
"""
    ...
    for scenes in seasons.values():
        ...
        for scene in scenes.values():
            ...
            for jump in jumps:
                ...
                if any(
                    str(condition or "").strip().startswith("__choice_")
            ...
            if game_data is None:
                if cache_manager.is_cache_valid(parser, SOURCE_DIRS):
                    ...
                    if (
                        ...
                        and data_has_expected_parser_version(cached_data)
                        and data_supports_scene_state_actions(cached_data)
                        and data_supports_menu_branch_jump_conditions(cached_data)
                    ):
                        game_data = cached_data
                    else:
                        print(
                            "Cache read failed or schema is stale, extracting game data..."
                        )
        ...
        if isinstance(options, list):
            for option_index, option in enumerate(options):
                if not isinstance(option, dict):
                    continue
                option_text_by_index[option_index] = str(
                    option.get("text") or ""
                ).strip()
...
def format_choice_reference(choice_line, option_index, choice_lookup):
    ...
    choice_meta = (
        choice_lookup.get(choice_line, {}) if isinstance(choice_lookup, dict) else {}
    )
    if choice_meta:
        choice_number = int(choice_meta.get("choice_index", 0)) + 1
        option_text = str(
            choice_meta.get("option_text_by_index", {}).get(option_index) or ""
        ).strip()
        if option_text:
...
def build_selected_choice_driver_text(
    choice_index, choice_line_number, selected_index, selected_text
):
    """Build a concise source string for a selected choice."""
    choice_number = int(choice_index) + 1
    option_number = int(selected_index) + 1
    choice_line_meta = (
        f" line {choice_line_number}" if isinstance(choice_line_number, int) else ""
    )
...
class SafeExpressionEvaluator(ast.NodeVisitor):
    ...
    def __init__(self, variables):
        ...
        # Whitelist of safe methods for simulation
        self.allowed_methods = {
            "append": lambda obj, arg: (
                obj.append(arg) if isinstance(obj, list) else None,
                obj,
            )[1],
            "grant": lambda obj, arg: True,  # achievement.grant
        }

    def visit_Constant(self, node):
        return node.value

    def visit_NameConstant(self, node):  # Python <3.8 compatibility
        return node.value
    ...
    def visit_Name(self, node):
        ...
        # Derived flags from legacy variables
    ...
    def visit_Attribute(self, node):
        ...
        # Special handling for persistent.xxx
        if isinstance(node.value, ast.Name) and node.value.id == "persistent":
            full_name = f"persistent.{attr}"
            return self.variables.get(full_name, False)

        # General attribute access
        if isinstance(obj, dict):
            return obj.get(attr, False)
        try:
    ...
    def visit_Call(self, node):
        # Only allow whitelisted method calls on known objects
        if isinstance(node.func, ast.Attribute):
            obj = self.visit(node.func.value)
            method = node.func.attr
            if method in self.allowed_methods and obj is not None:
                args = [self.visit(arg) for arg in node.args]
                return self.allowed_methods[method](obj, *args)
...
def apply_action(action, variable_state):
    """Apply one parsed action to mutable variable state."""
    ...
    if action_type == "assign":
        ...
        if method == "append" and target in variable_state:
            arg_value = safe_eval_expression(args, variable_state)
            if isinstance(variable_state[target], list):
                variable_state[target].append(arg_value)
            return {
                "type": "method_call",
                "target": target,
                "method": method,
        ...
        elif method == "grant" and target == "achievement":
            # Parse achievement name from args, e.g., '"E for Equality."'
            arg_value = safe_eval_expression(args, variable_state)
            if isinstance(arg_value, str):
                var_name = f"achievement_{arg_value.replace(' ', '_').replace('.', '')}"
                variable_state[var_name] = True
            return {
                "type": "method_call",
                "target": target,
                "method": method,
        ...
        target = str(action.get("target") or "").strip()
        default = action.get("default", "Nika")
        variable_state[target] = default
        return {
            "type": "input",
            "target": target,
            "value": default,
    ...
    elif action_type == "persistent_assign":
        target = str(action.get("target") or "").strip()
        op = str(action.get("op") or "=").strip()
        expr = str(action.get("expression") or "").strip()
        rhs = safe_eval_expression(expr, variable_state)
        current = variable_state.get(target, False)
        try:
            if op == "=":
                new_val = rhs
            elif op == "+=":
    ...
    elif action_type == "persistent_default":
        target = str(action.get("target") or "").strip()
        expr = str(action.get("expression") or "").strip()
        # Only set if not already defined
        if target not in variable_state:
            rhs = safe_eval_expression(expr, variable_state)
            variable_state[target] = rhs
            return {
        ...
        # Return statement: handled by call stack, no variable change
        return {"type": "return", "line_number": action.get("line_number")}

    return None


def materialize_playthrough(
    playthrough_data, data, include_dialogues=True, include_choice_states=False
):
    ...
    # Initialize lists that might be used in method calls
    if "Dating_List" not in variable_state:
        variable_state["Dating_List"] = []
    ...
    for source_season, source_label in selected_nodes:
        source_scene = (
            data.get("seasons", {}).get(source_season, {}).get(source_label, {})
        )
    ...
    # Call stack for simulating call/return
    call_stack = (
        []
    )  # each entry: (return_scene_node, return_line_number, variable_state_snapshot?)
    ...
    for scene_index, scene_ref in enumerate(scenes_input, start=1):
        ...
        if scene is None:
            missing_scenes.append(
                {"season": season, "label": label, "scene_index": scene_index}
            )
            continue
        ...
        if incoming_from_past:
            for incoming in incoming_from_past:
                ...
                if not evaluate_condition_list(
                    incoming.get("conditions", []), variable_state
                ):
                    continue
        ...
        if not scene_active:
            ...
            if previous_scene_node and scene_active_by_node.get(
                previous_scene_node, False
            ):
                previous_scene = (
                    data.get("seasons", {})
                    .get(previous_scene_node[0], {})
                    .get(previous_scene_node[1], {})
                )
                previous_file = (
                    str(previous_scene.get("file_path") or "")
                    .strip()
                    .replace("\\", "/")
                )
                current_file = (
                    str(scene.get("file_path") or "").strip().replace("\\", "/")
                )
                same_file = (
                    previous_file and current_file and previous_file == current_file
                )

                if same_file and not scene_has_true_jump_by_node.get(
                    previous_scene_node, False
                ):
                    scene_active = True

                # Handle same-file jump detours
                if (
                    same_file
                    and not scene_active
                    and scene_has_true_jump_by_node.get(previous_scene_node, False)
                ):
                    detour_target_label = str(
                        scene_true_jump_target_by_node.get(previous_scene_node) or ""
                    ).strip()
                    detour_target_node = resolve_jump_target_node(
                        previous_scene_node[0], detour_target_label
                    )
                    if detour_target_node:
                        detour_scene = (
                            data.get("seasons", {})
                            .get(detour_target_node[0], {})
                            .get(detour_target_node[1], {})
                        )
                        for detour_jump in detour_scene.get("jumps", []):
                            detour_target = resolve_jump_target_node(
                                detour_target_node[0], detour_jump.get("target")
                            )
                            if detour_target != scene_node:
                                continue

                            if evaluate_condition_list(
                                detour_jump.get("conditions", []), variable_state
                            ):
                                scene_active = True
                                break
        ...
        scene_calls = scene.get("calls", [])
        scene_call_screens = scene.get("call_screens", [])
        ...
        if isinstance(state_actions, list):
            state_actions = sorted(
                ...
                key=lambda action: (
                    ...
                    and isinstance(action.get("line_number"), int)
                    else 10**9
                ),
            )
        else:
        ...
        if matched_incoming_jump:
            ...
            if isinstance(source_node, tuple) and len(source_node) == 2:
                ...
                source_lookup = build_choice_line_lookup(
                    source_scene.get("choices", [])
                )
            ...
            if isinstance(incoming_conditions, list):
                for condition in incoming_conditions:
                    ...
                    if source_name:
                        scene_activation_condition_drivers.append(
                            f"scene gate from {source_name}: {described}"
                        )
                    else:
                        scene_activation_condition_drivers.append(
                            f"scene gate: {described}"
                        )
        ...
        if scene_override_info:
            ...
            override_choice = str(
                scene_override_info.get("choice_driver") or ""
            ).strip()
            if override_choice:
                if source_name:
                    scene_activation_condition_drivers.append(
                        f"scene jump from {source_name}: {override_choice}"
                    )
                else:
                    scene_activation_condition_drivers.append(
                        f"scene jump: {override_choice}"
                    )
            ...
            if isinstance(override_conditions, list):
                for condition in override_conditions:
                    described = describe_condition_for_diff(
                        condition, choice_line_lookup
                    )
                    if not described:
                        continue
                    scene_activation_condition_drivers.append(
                        f"scene jump condition: {described}"
                    )

        scene_activation_condition_drivers = dedupe_text_list(
            scene_activation_condition_drivers
        )
        ...
        if isinstance(scene_jumps, list):
            scene_jumps = sorted(
                scene_jumps,
                key=lambda jump: (
                    jump.get("line_number", 10**9) if isinstance(jump, dict) else 10**9
                ),
            )
        else:
            scene_jumps = []

        if isinstance(scene_calls, list):
            scene_calls = sorted(
                scene_calls,
                key=lambda call: (
                    call.get("line_number", 10**9) if isinstance(call, dict) else 10**9
                ),
            )
        else:
            scene_calls = []
        ...
        def apply_state_actions_until(max_line_exclusive=None):
            ...
            while state_action_cursor < len(state_actions):
                ...
                if (
                    max_line_exclusive is not None
                    and state_action_line >= max_line_exclusive
                ):
                    break
                ...
                if not evaluate_condition_list(
                    action.get("conditions", []), variable_state
                ):
                    continue
        ...
        def apply_scene_jumps_until(max_line_exclusive=None):
            ...
            if not scene_active or scene_exited_by_jump or scene_exited_by_call:
                return
            ...
            while scene_jump_cursor < len(scene_jumps):
                ...
                jump_line = (
                    ...
                    and isinstance(jump.get("line_number"), int)
                    else 10**9
                )
                ...
                if evaluate_condition_list(jump.get("conditions", []), variable_state):
                    ...
                    scene_exit_jump_info = {
                        ...
                        "conditions": (
                            list(jump.get("conditions", []))
                            if isinstance(jump.get("conditions"), list)
                            else []
                        ),
                    }
                    break

        def apply_scene_calls_until(max_line_exclusive=None):
            nonlocal scene_call_cursor, scene_exited_by_call, scene_exit_call_info

            if not scene_active or scene_exited_by_jump or scene_exited_by_call:
                return

            while scene_call_cursor < len(scene_calls):
                call = scene_calls[scene_call_cursor]
                call_line = (
                    call.get("line_number")
                    if isinstance(call, dict)
                    and isinstance(call.get("line_number"), int)
                    else 10**9
                )

                if max_line_exclusive is not None and call_line >= max_line_exclusive:
                    break
                ...
                if evaluate_condition_list(call.get("conditions", []), variable_state):
                    scene_exited_by_call = True
                    scene_exit_call_info = {
                        "line_number": call.get("line_number"),
                        "target": str(call.get("target") or "").strip(),
                        "conditions": (
                            list(call.get("conditions", []))
                            if isinstance(call.get("conditions"), list)
                            else []
                        ),
                    }
                    break
        ...
        for choice_index, choice in enumerate(choices):
            ...
            if choice_line_for_order is not None:
                ...
                apply_scene_calls_until(choice_line_for_order)

            scene_path_active = (
                scene_active and not scene_exited_by_jump and not scene_exited_by_call
            )
            ...
            if isinstance(parent_choice_line, int) and isinstance(
                parent_option_index, int
            ):
                parent_selected_index = selected_option_by_choice_line.get(
                    parent_choice_line
                )
            ...
            for option_index, option in enumerate(options):
                option_active = (
                    scene_path_active
                    and parent_branch_active
                    and evaluate_condition_list(
                        option.get("conditions", []), variable_state
                    )
                )
            ...
            requested_is_valid = isinstance(
                requested_index, int
            ) and 0 <= requested_index < len(options)
            choice_active = (
                scene_path_active and parent_branch_active and bool(available_indices)
            )
            ...
            if include_choice_states:
                choice_states[choice_key] = {
                    ...
                    "blocked_by_scene_call": scene_active and scene_exited_by_call,
                    "blocking_jump": scene_exit_jump_info,
                    "blocking_call": scene_exit_call_info,
                    ...
                    "requested_option_index": (
                        requested_index if requested_provided else None
                    ),
            ...
            scene_selected_choice_driver_texts = dedupe_text_list(
                scene_selected_choice_driver_texts
            )
            ...
            for action in selected_option.get("actions", []):
                ...
                if action_type == "jump":
                    ...
                    action_line = (
                        action.get("line_number")
                        if isinstance(action.get("line_number"), int)
                        else None
                    )
                    ...
                    if isinstance(action.get("conditions"), list):
                        ...
                        action_condition_sets = jump_conditions_by_signature.get(
                            (action_line, action_target), []
                        )
                    ...
                    if target_node:
                        # Push current scene onto call stack
                        call_stack.append(
                            {
                                "return_scene": scene_node,
                                "return_line": action.get("line_number"),
                                "state_snapshot": dict(variable_state),  # optional
                            }
                        )
                        scene_exited_by_call = True
                        scene_exit_call_info = {
            ...
            for dialogue in dialogues:
                ...
                if isinstance(dialogue_conditions, list):
                    for condition in dialogue_conditions:
                        ...
                        if described:
                            dialogue_condition_drivers.append(
                                f"line condition: {described}"
                            )

                influence_conditions = dedupe_text_list(
                    [*scene_activation_condition_drivers, *dialogue_condition_drivers]
                )
        ...
        scene_true_call = scene_active and scene_exited_by_call

        scene_has_true_jump_by_node[scene_node] = scene_true_jump or scene_true_call

        scene_exit_target = ""
        if scene_true_jump or scene_true_call:
            if isinstance(scene_exit_jump_info, dict):
                scene_exit_target = str(
                    scene_exit_jump_info.get("target") or ""
                ).strip()
            elif isinstance(scene_exit_call_info, dict):
                scene_exit_target = str(
                    scene_exit_call_info.get("target") or ""
                ).strip()

        scene_true_jump_target_by_node[scene_node] = scene_exit_target

        # Handle return from call
        if scene_return_encountered and call_stack:
            # Pop the call stack and potentially resume the calling scene
            # In our simplified model, we just note the return; actual resumption would
            # require the caller scene to be later in the playthrough list.
            call_stack.pop()
            # For now, we don't automatically jump back; user must include the caller again.
            # This is a limitation of the static playthrough approach.

    active_scene_count = sum(
        1 for scene_state in scene_states.values() if scene_state.get("active")
    )
...
def build_continuous_dialogue_diff(
    ...
    left_lines = [
        diff_line_payload(dialogue, index + 1)
        for index, dialogue in enumerate(left_dialogues)
    ]
    right_lines = [
        diff_line_payload(dialogue, index + 1)
        for index, dialogue in enumerate(right_dialogues)
    ]
        ...
        if tag in ("delete", "replace"):
            for offset in range(i2 - i1):
                current_lines.append(
                    {"type": "remove", "left": left_lines[i1 + offset], "right": None}
                )

        if tag in ("insert", "replace"):
            for offset in range(j2 - j1):
                current_lines.append(
                    {"type": "add", "left": None, "right": right_lines[j1 + offset]}
                )
...
def chapter_file_sort_key(file_path):
    ...
    chapter_match = re.match(
        r"^chapter(?P<major>\d+)(?:(?:[x\.])(?P<minor>\d+))?(?P<suffix>.*)$", stem_lower
    )
    if chapter_match:
...
def build_timeline_entries(data):
    ...
    for season, scenes in seasons.items():
        for label_name, scene in scenes.items():
            ...
            meta_matches = (
                isinstance(label_meta, dict) and label_meta.get("season") == season
            )
...
def get_timeline():
    ...
    for entry in entries:
        timeline.append(
            {**entry, "reachable": (entry["season"], entry["label"]) in reachable}
        )
...
def compare_playthroughs():
    ...
    missing_fields = [
        name
        for name in ("season1", "label1", "season2", "label2")
    ...
    return jsonify(
        {
            ...
            "scene1_info": {
                "season": season1,
                "label": label1,
                "total_dialogues": len(scene1_dialogues),
            },
            "scene2_info": {
                "season": season2,
                "label": label2,
                "total_dialogues": len(scene2_dialogues),
            },
        }
    )
...
def compare_full_playthroughs():
    ...
    if not isinstance(left_data, dict) or not isinstance(right_data, dict):
        return (
            jsonify(
                {"error": "Both left_playthrough and right_playthrough are required"}
            ),
            400,
        )

    if not isinstance(left_data.get("scenes", []), list) or not isinstance(
        right_data.get("scenes", []), list
    ):
    ...
    return jsonify(
        {
            ...
            "samples": {"common": [], "only_in_left": [], "only_in_right": []},
            "truncated": {
                "common": False,
                "only_in_left": False,
                "only_in_right": False,
...
def preview_full_playthrough():
    ...
    preview = materialize_playthrough(
        playthrough_data, data, include_dialogues=False, include_choice_states=True
    )
...
if __name__ == "__main__":
    ...
    if should_auto_open_browser(debug_enabled):
        delay_seconds = max(
            0.1, float(os.environ.get("SG_AUTO_OPEN_DELAY_SECONDS", "1.0"))
        )
